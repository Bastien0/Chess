...
void Grid::move(int coord[2], Chessman& chessman, string promotion){
    ...
    vector<Chessman>::const_iterator it = V.begin();
    for (; it != l.end(); ++it)
...
bool Grid::isChessed(Chessman& chessman, int x, int y){
    ...
    for (int i = 0; i < 8; i++){
        for (int j = 0; j < 8; j++){
            ...
            if (!this->isVoid(i,j) && chessman.getIsWhite() != this->(i,j).getIsWhite() \
                    && l.find(this->king_position(chessman.getIsWhite())) != l.end()){
